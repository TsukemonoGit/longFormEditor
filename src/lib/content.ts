...
import { nip19Regex, urlRegex, nipRegex, relayRegex, invoiceRegex } from './regex';

// インターフェース定義
export interface Part {
	type: 'nip19' | 'url' | 'emoji' | 'hashtag' | 'nip' | 'relay' | 'text' | 'quote' | 'invoice';
...
const findEarliestMatch = <T>(
	...
	return earliestIndex !== -1 && foundItem ? { index: earliestIndex, item: foundItem } : null;
};

// URLの括弧チェック用関数
const findLastUnpairedParenIndex = (url: string, openChar: string, closeChar: string): number => {
...
// メインのテキスト解析関数
export async function parseText(input: string, tags: string[][]): Promise<Part[]> {
	const parts: Part[] = [];
	...
	const hashtagSet: string[] = Array.from(
		new Set(tags.filter((tag) => tag[0] === 't').map((tag) => `#${tag[1].toLowerCase()}`))
	).sort((a, b) => b.length - a.length);
	...
	while (remainingText.length > 0) {
		...
		const matches = [
			...
			},
			{
				...
				process: (m: RegExpMatchArray) => {
					const url = m[0];
					// 括弧のペアをチェック
					let lastUnpairedParenIndex = findLastUnpairedParenIndex(url, '(', ')');
					if (lastUnpairedParenIndex === url.length) {
						// 日本語の括弧もチェック
						lastUnpairedParenIndex = findLastUnpairedParenIndex(url, '（', '）');
					}
			...
			{
				type: 'emoji',
				match: findEarliestMatch(remainingText, emojiSet, (emoji, text) => text.indexOf(emoji)),
				process: (match: IndexResult<string>) => {
					const emojiContent = match.item.slice(1, -1); // コロンを削除
					const matchingTag = tags.find((tag) => tag[0] === 'emoji' && tag[1] === emojiContent);
					return {
						type: 'emoji' as const,
						url: matchingTag?.[2],
			...
			{
				type: 'hashtag',
				match: findEarliestMatch(remainingText.toLowerCase(), hashtagSet, (hashtag, text) =>
					text.indexOf(hashtag)
				),
				process: (match: IndexResult<string>) => {
					const originalHashtag = remainingText.slice(match.index, match.index + match.item.length);
					return {
			...
			},
			{
				...
				process: (m: RegExpMatchArray) => ({
					type: 'nip' as const,
					content: m[0],
					url: `https://github.com/nostr-protocol/nips/blob/master/${m[0].slice(4)}.md` // "nip-" プレフィックスを削除
				})
			},
			{
				type: 'relay',
		...
		const validMatches = matches
			.map((match) => {
				...
				const index =
					match.match instanceof Array ? remainingText.indexOf(match.match[0]) : match.match.index;
		...
		// 処理済みの部分をスキップ
		const matchLength = match instanceof Array ? match[0].length : match.item.length;
